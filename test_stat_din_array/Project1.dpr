...
begin
  ...
  repeat
    ...
    readln(s1);
    s2 := s1;
    writeln('Adress shortstrings: ', int32(@s1[1]), ' ', int32(@s2[1]),
      #10, s2);
    s4 := s1;
    ...
    for i := 1 to 1000 do
      m4[i] := random(256);
    writeln('Adress din array (before copy): ', int32(@m4[1]), ' ',
      int32(@m5[1]));
    m5 := m4;
    writeln('Adress din array (after copy): ', int32(@m4[1]), ' ',
      int32(@m5[1]));
    m5[0] := 0;
    writeln('Adress din array (after m5[0]:=0): ', int32(@m4[1]), ' ',
      int32(@m5[1]));
    readln;
    eq := true;
    for i := 1 to 1000 do
